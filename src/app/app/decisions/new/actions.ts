...
function redirectWithMessage(
  ...
  redirect(`/app/decisions/new?${params.toString()}`);
}
...
export async function createDecision(formData: FormData) {
  ...
  if (approverIds.length > 0) {
    ...
    const decisionLink = `${getBaseUrl()}/app/decisions/${decision.id}`;
    const recipients =
    ...
    if (failed) {
      redirect(
        `/app/decisions/${decision.id}?error=${encodeURIComponent(
  ...
  redirect(`/app/decisions/${decision.id}`);
}
